...
class HttpRequestHandler:
    ...
    def http_event_received(self, event: H3Event) -> None:
        if isinstance(event, DataReceived):
            logging.info(f"{RECEIVER} HTTP/3 Request data received")
            ...
            logging.info(f"{RECEIVER} HTTP/3 Request headers received")
            self.queue.put_nowait(
    ...
    async def receive(self) -> Dict:
        logging.info(f"{RECEIVER} HTTP/3 Request received")
        return await self.queue.get()

    async def send(self, message: Dict) -> None:
        if message["type"] == "http.response.start":
            logging.info(f"{RECEIVER} Sending HTTP/3 Response headers")
            ...
            logging.info(f"{RECEIVER} Sending HTTP/3 Response data")
            ...
            logging.info(f"{RECEIVER} Pushing HTTP/3 Response")
            request_headers = [
                (b":method", b"GET"),
                (b":scheme", b"https"),
