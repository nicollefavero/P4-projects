        ...
        if(hdr.ipv4.isValid()) {
            ...
            if(hdr.udp.isValid()) {
                ...
                if(check_ports.apply().hit) {
                    // Drop packets from server to malicious agent to simulate an firewall or ip spoofing
                    if(direction == 0) {
                        drop();
                    }

                    // Drop HANDSHAKE packets from attacker to simulate an QUIC flood attack
                    if(direction == 1) {
                    ...
                    // For the cases where direction == 2 or direction == 3, just forward regularly as possible.
                }
            }
        }
    }
