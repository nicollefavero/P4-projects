...
MAKEFILE_PATH = "/home/p4/Repositories/P4-projects/QUIC-Tr4ck-Impl/3-Endpoints/QUIC-flood/p4-src/Makefile"
SCRIPTS_PATH = "/home/p4/Repositories/P4-projects/QUIC-Tr4ck-Impl/3-Endpoints/QUIC-flood"

def generate_request_time_log(output_dir, times, interval, num_requests, burst_size, burst_interval, burst_count):
    output_dir = f"{output_dir}/r{num_requests}-i{interval}"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    file_name = f"{output_dir}/s{burst_size}-c{burst_count}-t{burst_interval}".replace(".", "_")
...
class MininetProc:
    ...
    def run_client(self, interval, num_requests):
        ...
        for i in range(num_requests):
            t_start = time.time()
            self.proc.sendline(f"h3 python3 {SCRIPTS_PATH}/quic-src/send.py --ca-certs {SCRIPTS_PATH}/quic-src/tests/pycacert.pem https://10.0.2.2:4433/")
            self.proc.expect("mininet> ", timeout=None)
            t_end = time.time()
...
def get_args():
    ...
    parser.add_argument("-c", "--burst-count", type=int)
    parser.add_argument("-t", "--burst-interval", type=float)
    parser.add_argument("-r", "--requests-number", type=int, default=2)
...
def main():
    ...
    if (burst_size is None) or (burst_count is None) or (burst_interval is None):
        mininet_proc.run_server()
        time.sleep(2)
        conn_times = mininet_proc.run_client(interval, num_requests)
        generate_request_time_log(output_client, conn_times, interval, num_requests, 0, 0, 0)
